...
namespace AnPhuocShop.Api.Controllers
{
    ...
    public class AnPhuocsController : ControllerBase
    {
        ...
        [HttpPut()]
        public async Task<ActionResult<AnPhuoc>> UpdateAnPhuoc(AnPhuoc anphuoc)
        {
            try
            {
                var anphuocToUpdate = await anPhuocRepository.GetAnPhuoc(anphuoc.AnPhuocId);
                if (anphuocToUpdate == null)
                    return NotFound($"Khong tim thay san pham Id = {anphuoc.AnPhuocId}");
        ...
        [HttpGet("{search}")]
        public async Task<ActionResult<IEnumerable<AnPhuoc>>> Search (string name)
        {
            try
            {
